...
Vertex.prototype = {
    node_loop_angle: function() {
        var angles = [], angle = 0, i, diff, bestdiff = 0, edge, npos,
            thispos = this.pos, neighbors_list = neighbors_of(this);
    ...
    draw_loop: function() {
        ...
        circle(this.pos.x + 1.5 * Math.cos(angle) * NODE_RADIUS,
            this.pos.y - 1.5 * Math.sin(angle) * NODE_RADIUS,
            2 * NODE_RADIUS, true);
    },
    run: function() {
...
Edge.prototype = {
    ...
    draw_multi: function(){
        ...
        for (i = -(this.multi-1) / 2; i <= (this.multi-1) / 2; i += 1){
            ...
            bezier(pos1.x, pos1.y, control.x, control.y, control.x, control.y,
                pos2.x, pos2.y);
            if (DIRECTED) {
    ...
    dec_mult: function() {
        if (this.multi > 0) this.multi -= 1;
        if (this.multi === 0) remove_edge(this);
    }
};
    ...
    for (i = 0; i < nodes.length; i += 1) {
        if (nodes[i] !== hit_node) {
            ...
            if (maximize) {
                ...
                newp = {
                  x: (SIZE.x / 2) + ((SIZE.x * 9 / 10) * (pos.x - min_x) -
                      (SIZE.x * 9 / 20) * width) / scaling_factor,
                  y: (SIZE.y / 2) + ((SIZE.y * 9 / 10) * (pos.y - min_y) -
                      (SIZE.y * 9 / 20) * height) / scaling_factor
                };
            } else {
                newp = {
...
function split(edge) {
    ...
    newpos = scalarmi(1 / 2,
        vectoradd(enodes.node1.get_pos(), enodes.node2.get_pos()));
